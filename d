...
findChestAccess = function(target)
    local accessPositions = {
        {x = target.x - 1, z = target.z, dir = 1},  -- Left
        {x = target.x + 1, z = target.z, dir = 3},  -- Right
        {x = target.x, z = target.z - 1, dir = 0},  -- Front
        {x = target.x, z = target.z + 1, dir = 2}   -- Back
    }

    for _, access in ipairs(accessPositions) do
        moveTo({x = access.x, y = target.y, z = access.z}) -- Move adjacent

        while pos.dir ~= access.dir do turnRight() end  -- Face the chest position

        -- ✅ Check if there's a block in front before proceeding
        if turtle.detect() then
            print("❌ Side blocked! Trying another...")
        else
            -- ✅ Turn until front peripheral is a chest
            for _ = 1, 4 do
                local front = peripheral.getType("front")
                if front and front:match("chest") then
                    print("✅ Chest found! Ready to interact.")
                    return true
                end
                turnRight()
            end

            print("❌ No chest detected! Retrying another side...")
        end
    end

    -- If all sides are blocked, wait and retry
    print("🚧 All sides blocked! Waiting for access...")
    sleep(2)
    return findChestAccess(target)  -- Retry recursively
end
